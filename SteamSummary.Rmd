...
## Grouping Genres into Larger Umbrella Genres
```{r}
...
## Removal Of Owners Variable Since we are now predicting on Successful/Unsuccessful
```{r}
...
boxplot(steam$price~steam$categories,
        ...
        ylab = "Price (dollars)")
```
The above boxplot shows how the spread of price values for video games differ across the main categories of games. It appears that the spread of prices are generally the same, yet single player video games appear to have the most amount of outliers compared to other main categories.
```{r}
ggplot(steam,aes(x = genres,y = price)) + geom_boxplot() + 
  labs(title = "Boxplots of Price By Genre",x = "Genres",y = "Price (dollars)")
```
From the above boxplot we can determine that the spread of prices are generally larger for indie games and "other" games than when compared with the remaining main genres.  This is interesting, as perhaps the increased price of indie games and miscelaneous games may have an effect on the amount of copies bought, thus effecting its ability to sell over 1,000,000 copies.  However, previous plots illustrating the relationship between price and successful game suggest there may not be a significant relationship between the two variables.
