---
title: "Estrada-Rand_Filce_Hoskins_Final_Proj_CP2"
...
### K-Fold Cross Validation
nfolds <- 10
preds_KFold_CV_DF <- data.frame(folds = steam$folds,rownum = steam$row_num,preds_KFold_CV = rep(NA,nrow(steam)))

# for loop for K-Fold CV
for(i in 1:nfolds){
  mod <- lm(successfulGame ~ ., 
            data = steam[steam$folds != i,])
  preds <- predict(mod, 
                   newdata = steam[steam$folds == i,])
